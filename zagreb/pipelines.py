...
class ZagrebPipeline:

    # Database setup
    conn = sqlite3.connect('zagreb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `zagreb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM zagreb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO zagreb (date, title, link, content)"
